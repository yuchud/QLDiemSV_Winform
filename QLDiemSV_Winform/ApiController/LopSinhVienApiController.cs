    ...
    internal class LopSinhVienApiController
    {
        ...
        public static HttpStatusCode PostLopSinhVien(LopSinhVienDTO lopSinhVien)
        {
            using (var httpClient = new HttpClient())
            {
                ...
                HttpResponseMessage response = httpClient.PostAsync($"{Api_LopSinhVien_Url}", new StringContent(jsonLopSinhVien, System.Text.Encoding.UTF8, "application/json")).Result;
        ...
        public static HttpStatusCode PutLopSinhVien(LopSinhVienDTO lopSinhVien)
        {
            ...
            {
                string jsonLopSinhVien = JsonConvert.SerializeObject(lopSinhVien);
                HttpResponseMessage response = httpClient.PutAsync($"{Api_LopSinhVien_Url}/{lopSinhVien.MaLopSv}", new StringContent(jsonLopSinhVien, System.Text.Encoding.UTF8, "application/json")).Result;
                return response.StatusCode;
            }
        }
        ...
        public static EnumCode.ApiDeleteResult DeleteLopSinhVien(int maLopSinhVien)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    HttpResponseMessage httpResponse = httpClient.DeleteAsync($"{Api_LopSinhVien_Url}/{maLopSinhVien}").Result;
